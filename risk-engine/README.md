...
I did this using four individual methods, and then backtested those methods against real price history verify they held up as expected.

This project was built for ground up, including creating a inverse normal quantile function by hand to have a closer look and develop a greater understanding of the statistic behind it and how we can actually compute it. SciPy and known results are used to verify my own computations.
...
3. Fat-tailed (Student-t) Monte Carlo - this method fits a Student-t to the return data and simulates from it which catches the heavy tails real returns, 
...
1. The Distribution test: Jarque-Bera firmly rejects normality, by looking at the data we can see that kurtosis ≈ +6.43 (which means heavily weighted tails), and a fitted Student-t has only ≈ 3.3 degrees of freedom. This evidence suggests that it is strongly fat-tailed.
...
3. Backtest (out-of-sample): by walking forward through history and counting the breaches, the 95% VaR is well calibrated (4.63% breach rate vs a 5% target), but the 99% VaR over breaches it target (1.59% vs a 1% target) - this helps us be confirming the tail underestimate on the real data. By switching the 99% model to the fat-tailed version (using Student-t) improves the breach rate to 1.19% which is much closer to the target.
